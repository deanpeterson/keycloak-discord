...
public class DiscordIdentityProviderConfig extends OAuth2IdentityProviderConfig {
    ...
    public void setBotToken(String botToken) {
        getConfig().put("botToken", botToken);
    }
    ...
    public String getGuildsToJoin() {
        return getConfig().get("guildsToJoin");
    }

    public void setGuildsToJoin(String guildsToJoin) {
        getConfig().put("guildsToJoin", guildsToJoin);
    }

    public boolean hasGuildsToJoin() {
        String guilds = getConfig().get("guildsToJoin");
        return guilds != null && !guilds.trim().isEmpty();
    }

    public Set<String> getGuildsToJoinAsSet() {
        if (hasGuildsToJoin()) {
            String guilds = getConfig().get("guildsToJoin");
            return Arrays.stream(guilds.split(",")).map(x -> x.trim()).collect(Collectors.toSet());
        }
